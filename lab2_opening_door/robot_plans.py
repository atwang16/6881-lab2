...
class OpenLeftDoorPlan(PlanBase):
    ...
    def CalcKinematics(
            self, X_L7E, l7_frame, world_frame, tree_iiwa, context_iiwa, t_plan):
        '''
        ...
        X_WHr.set_rotation(np.array([[np.cos(angle), np.sin(angle), 0.],
                                     [-np.sin(angle), np.cos(angle), 0.],
                                     [0., 0., 1.]]))
        X_WHr.set_translation(p_WC_handle + [-r_handle * np.sin(angle), -r_handle * np.cos(angle), 0.])
        #------------------------------------------------------------------------#
        X_HrW = X_WHr.inverse()
...
class OpenLeftDoorPositionPlan(OpenLeftDoorPlan):
    ...
    def CalcPositionCommand(self, t_plan, q_iiwa, Jv_WL7q, p_HrQ, control_period):
        ...
        alpha = 0.3 * np.eye(3)
        e = np.zeros(6)
        ae_xyz = np.dot(alpha, p_HrQ)
        e[3:] = ae_xyz
        Jv_WL7q_inv = np.linalg.pinv(Jv_WL7q)
        q_des = np.dot(Jv_WL7q_inv, e)
        q_ref = q_iiwa + q_des * control_period
        #------------------------------------------------------------------------#
        return q_ref
...
class OpenLeftDoorImpedancePlan(OpenLeftDoorPlan):
    ...
    def CalcTorqueCommand(self, t_plan, Jv_WL7q, p_HrQ):
        ...
        beta *= 10
        e = np.zeros(6)
        be_xyz = np.dot(beta, p_HrQ)
        e[3:] = be_xyz
        t_ff = np.dot(np.transpose(Jv_WL7q), e)
        #------------------------------------------------------------------------#
        return t_ff
