(define (stream manipulation-station)
  ...
  (:stream plan-pull
    ...
    :inputs (?robot ?door ?conf1 ?conf2)
    :domain (and (Robot ?robot)
                 (Door ?door)
                 (Conf ?door ?conf1)
                 (Conf ?door ?conf2))
    :outputs (?robot_conf1 ?robot_conf2 ?traj)
    :certified (and (Pull ?robot ?robot_conf1 ?robot_conf2 ?door ?conf1 ?conf2 ?traj)
                    (Conf ?robot ?robot_conf1)
                    (Conf ?robot ?robot_conf2)
                    (Traj ?traj))
    ;;;;; END YOUR CODE HERE ;;;;;
  )
